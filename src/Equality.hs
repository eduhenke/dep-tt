...
import Unbound.Generics.LocallyNameless (unbind, unbind2, Subst (substs))
...
equateArgs [] [] = return ()
equateArgs a1 a2 = 
  err ["Expected", show (length a2),
       "but found", show (length a1)]
...
whnf (Case scrut cases) = do
  nf <- whnf scrut
  case nf of
    (DCon d args) -> f cases where
      f (Match bnd : cases) = (do
        (pat, body) <- unbind bnd
        ss <- patternMatches nf pat 
        whnf $ substs ss body) `catchError` \_ -> f cases
      f [] = err $ ["Internal error: couldn't find a matching",
                    "branch for", show nf, "in"] ++ map show cases
...
unify ns tx ty | trace ("unify: " ++ show ns ++ show tx ++ show ty) False = undefined
unify ns tx ty = do
  txnf <- whnf tx
  tynf <- whnf ty
